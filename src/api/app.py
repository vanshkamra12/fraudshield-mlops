...
async def predict(txn: TransactionInput):
    ...
    try:
        ...
        fraud_score = model.predict(df_transformed.values.astype(np.float32))[0]
        ...
        logger.info(f"Prediction logged: {transaction_id} → score={fraud_score:.4f}, label={predicted_label}")
...
async def feedback(transaction_id: str, actual_label: int):
    ...
    try:
        ...
        logger.info(f"Feedback received: {transaction_id} → actual_label={actual_label}")
